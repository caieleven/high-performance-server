...
// 子进程运行的函数，参数idx指出该子进程处理的客户连接的编号，users是保存所有客户连接数据的数组，参数share_mem指出共享内存地址的起始地址
        ...
        for ( int i = 0; i < number; i++ ){
            ...
            if( sockfd == listenfd ){
                ...
                if( pid < 0 ){
                    ...
                    // 记录新的客户连接在数组users中的索引值，建立pid和该索引值之间的映射关系
